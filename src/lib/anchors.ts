...
import { encryptWithKey, decryptWithKey, generateKey } from './encryption';
import auditTrail, { type APIResponseInfo } from './auditTrail';
...
export interface AnchorProcessingTime {
  deposit: string;
  withdrawal: string;
}
...
export interface AnchorDefinition {
  ...
  fees: AnchorFees;
  processingTime: AnchorProcessingTime;
  status: string;
}
...
export interface AnchorRateOption extends AnchorDefinition {
...
class AnchorService {
  ...
  private authSessionStorePrefix = 'sep10-anchor-session:';
  private supportedAnchors: AnchorDefinition[];
  ...
  getAvailableAnchors(filters: AnchorFilterOptions = {}): AnchorDefinition[] {
   ...
   */
  getAnchor(anchorId: string): AnchorDefinition | undefined {
   ...
   */
  calculateFees(anchorId: string, type: 'deposit' | 'withdrawal', amount: number): FeeCalculation {
   ...
   */
  getBestRates(asset: string, amount: number, type: 'deposit' | 'withdrawal'): AnchorRateOption[] {
    const anchors = this.getAvailableAnchors({ asset, status: 'active' });
    const rates: AnchorRateOption[] = [];
   ...
   */
  calculateAnchorScore(anchor: AnchorDefinition, feeCalculation: FeeCalculation): number {
    let score = 100;
   ...
   */
  generateMethodSpecificInstructions(
   ...
   * @returns {object}
   */
  parseToml(tomlText: string): StellarTomlFields {
  ...
  async getWebAuthEndpoint(anchor: AnchorDefinition | undefined): Promise<string> {
    if (!anchor) {
  ...
  async requestChallengeTransaction(anchorId, accountPublicKey, network = 'TESTNET') {
    ...
    auditTrail.logAPICall(url.toString(), 'GET', { account: accountPublicKey }, {
      status: response.status,
      responseTime: duration,
      ...(Object.keys(responseData).length > 0 ? { responseData } : {}),
    } as APIResponseInfo & Record<string, unknown>);
  ...
  async submitChallengeTransaction(anchorId, signedTransactionXdr) {
    ...
    auditTrail.logAPICall(endpoint, 'POST', { transaction: '[REDACTED]' }, {
      status: response.status,
      responseTime: duration,
      ...(Object.keys(responseBody).length > 0 ? { response: responseBody } : {}),
    } as APIResponseInfo & Record<string, unknown>);
