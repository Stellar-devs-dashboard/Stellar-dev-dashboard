...
export async function fetchOrderBook(
  ...
  const cached = cache.get<OrderBookResult>(key);
...
export async function fetchTrades(
  ...
  network: NetworkName = "testnet",
  limit = 50,
): Promise<StellarSdk.Horizon.ServerApi.TradeRecord[]> {
  const server = getServer(network);
  const key = cache.generateKey("trades", { b: baseAsset.toString(), c: counterAsset.toString(), network });
  const cached = cache.get<StellarSdk.Horizon.ServerApi.TradeRecord[]>(key);
  if (cached) return cached;
...
export async function fetchAllAssets(network: NetworkName = "testnet", limit = 200) {
  const server = getServer(network);
...
export async function fetchLiquidityPools(
  network: NetworkName = "testnet",
  limit = 50,
  assetFilter: string | StellarSdk.Asset | Array<string | StellarSdk.Asset> | null = null,
): Promise<EnrichedLiquidityPool[]> {
  const key = cache.generateKey("pools", { network, limit, assetFilter });
  const cached = cache.get<EnrichedLiquidityPool[]>(key);
  if (cached) return cached;
  ...
  const result = (pools._embedded?.records || pools.records || []).map((pool) =>
    enrichPool(pool as Record<string, unknown>),
  );
  cache.set(key, result, TTL.POOLS);
  return result;
}

export async function fetchLiquidityPoolsByAssetPair(
 ...
 */
export async function fetchPoolById(poolId: string, network: NetworkName = "testnet") {
  ...
  const result = enrichPool(pool as Record<string, unknown>);
 ...
 */
export async function fetchPoolTrades(poolId: string, network: NetworkName = "testnet", limit = 50) {
  const server = getServer(network);
 ...
 */
export async function fetchPoolOperations(poolId: string, network: NetworkName = "testnet", limit = 50) {
...
export async function fetchAccountLiquidityPoolPositions(publicKey: string, network: NetworkName = "testnet"): Promise<AccountLiquidityPosition[]> {
  const server = getServer(network);
  const key = cache.generateKey("account-pool-positions", { publicKey, network });
  const cached = cache.get<AccountLiquidityPosition[]>(key);
  if (cached) return cached;
  ...
  const result = await Promise.all(
    poolShareBalances.map(async (balance) => {
      ...
      const poolBalance = balance as StellarSdk.Horizon.HorizonApi.BalanceLineLiquidityPool & {
      ...
      return {
        ...
        limit: balance.limit,
        buyingLiabilities: poolBalance.buying_liabilities,
        sellingLiabilities: poolBalance.selling_liabilities,
        sharePercent,
        pool,
      };
    })
...
export async function fetchAccountLiquidityPoolHistory(
  ...
  const cached = cache.get<StellarSdk.Horizon.ServerApi.OperationRecord[]>(key);
  if (cached) return cached;
...
interface HorizonEmbeddedResponse {
  _embedded?: { records?: unknown[] };
  records?: unknown[];
}

async function horizonGet(network: NetworkName, path: string): Promise<HorizonEmbeddedResponse> {
  ...
  return response.json() as Promise<HorizonEmbeddedResponse>;
}
...
export interface AccountLiquidityPosition {
  poolId: string | undefined;
  shares: number;
  balance: string;
  limit: string | undefined;
  buyingLiabilities: string | undefined;
  sellingLiabilities: string | undefined;
  sharePercent: number;
  pool: EnrichedLiquidityPool | null;
}

/**
 * Add computed fields to a raw Horizon pool record.
 */
export function enrichPool(pool: Record<string, unknown>): EnrichedLiquidityPool {
  const reserves = (pool.reserves as Array<{ amount?: string; asset?: string }> | undefined) || [];
  const totalShares = parseFloat(String(pool.total_shares ?? '')) || 0;
  const totalTrustlines = Number(pool.total_trustlines ?? 0);

  // Parse reserve amounts
  const reserveA = reserves[0] ? parseFloat(reserves[0].amount ?? '0') : 0;
  const reserveB = reserves[1] ? parseFloat(reserves[1].amount ?? '0') : 0;
  ...
  const feeBps = Number(pool.fee_bp ?? 30);
