export interface BuildContractWorkspaceOptions {
  contractName?: string;
}

export interface SimulateSorobanTestSettings {
  profile?: string;
}

export interface DeploymentPlanConfig {
  network?: string;
...
export function getContractTemplateById(templateId: string): ContractTemplate | null {
  return CONTRACT_TEMPLATES.find((template) => template.id === templateId) || null
}

export function buildContractWorkspace(templateId: string, options: BuildContractWorkspaceOptions = {}) {
...
export function simulateSorobanTests(
  sourceCode: string,
  testCode: string,
  settings: SimulateSorobanTestSettings = {},
) {
  const issues = []
...
export function generateDeploymentPlan(config: DeploymentPlanConfig = {}) {
