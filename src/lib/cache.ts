...
export type CacheFallbackSource = 'memory' | 'memory-stale' | 'indexeddb' | 'miss';

export interface GetWithFallbackResult<T = unknown> {
  value: T | null;
  stale: boolean;
  source: CacheFallbackSource;
}

export type CacheSubscriber<T = unknown> = (value: T) => void;
...
class LRUNode<T = unknown> {
  ...
  constructor(key: string, value: T | null, expiresAt: number, tags: string[] = []) {
    this.key       = key;
    this.value     = value;
    ...
    this.tags      = tags;
    this.prev      = null as unknown as LRUNode<T>;
    this.next      = null as unknown as LRUNode<T>;
  }
}
...
export class Cache {
  ...
  private _insertAfterHead(node: LRUNode): void {
  ...
  private _removeTags(node: LRUNode): void {
   ...
   */
  async getWithFallback<T = unknown>(key: string): Promise<GetWithFallbackResult<T>> {
    const node = this._map.get(key);
    const now  = Date.now();
    ...
    if (node) {
      ...
      if (now <= node.expiresAt) {
        this._hits++;
        return { value: node.value as T, stale: false, source: 'memory' };
      }
      ...
      if (now <= node.expiresAt + staleWindow) {
        this._hits++;
        return { value: node.value as T, stale: true, source: 'memory-stale' };
      }
    }
    ...
    if (this.persist) {
      try {
        const stored = await getStoredValue(key) as StoredCacheEntry<T> | null;
  ...
  async swr<T>(
    ...
    const { value, stale } = await this.getWithFallback<T>(key);
   ...
   * @returns unsubscribe
   */
  subscribe<T>(key: string, cb: CacheSubscriber<T>): () => void {
    if (!this._subscribers.has(key)) this._subscribers.set(key, new Set());
    this._subscribers.get(key)!.add(cb as CacheSubscriber);
    return () => { this._subscribers.get(key)?.delete(cb as CacheSubscriber); };
  }
  ...
  private _evictNode(node: LRUNode): void {
    this._detach(node);
