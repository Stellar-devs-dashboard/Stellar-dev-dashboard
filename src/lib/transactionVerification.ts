...
import { fetchAccount, fetchTransactions, NETWORKS, type NetworkName } from "./stellar";
...
async function checkUnusualPatterns(operations, sourceAccount, network) {
  ...
  try {
    ...
    if (recentTxs.length >= 10) {
      ...
      const timeDiff = newestTx.getTime() - oldestTx.getTime();
 ...
 */
export async function analyzeAccountActivity(publicKey: string, network: NetworkName = "testnet") {
  const warnings = [];
  const info = [];
