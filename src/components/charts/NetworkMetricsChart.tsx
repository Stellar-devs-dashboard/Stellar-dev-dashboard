...
export default function NetworkMetricsChart() {
  ...
  const [ledgerData, setLedgerData] = useState<LedgerMetricRow[]>([])
  const [loading, setLoading] = useState(false)
  const [refreshTick, setRefreshTick] = useState(0)
  const [hidden, setHidden] = useState<Set<SeriesId>>(() => new Set())
  const [selected, setSelected] = useState<LedgerMetricRow | null>(null)
  const [exportOpen, setExportOpen] = useState(false)

  const txChartRef = useRef<HTMLDivElement>(null)
  ...
  const isHidden = (id: SeriesId) => hidden.has(id)
...
function toggleStyle(color: string, isOff: boolean): React.CSSProperties {
  return {
    display: 'inline-flex',
    alignItems: 'center',
    gap: 4,
