...
import { ContractDeployer, type DeploymentTimelineEntry, type DeploymentResult } from '../../lib/deployment/ContractDeployer';
...
export default function ContractDeployerView() {
  ...
  const deployNetwork: 'testnet' | 'mainnet' = isMainnet ? 'mainnet' : 'testnet';
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (!result.isSimulation && result.receipt) {
        ...
        nextResult = {
          ...
          receipt: {
            ...
            status: 'confirmed',
            statusHistory: [
              ...result.receipt.statusHistory.map((entry): DeploymentTimelineEntry =>
                entry.id === 'submit'
                  ? {
                      ...entry,
                      status: 'complete',
      ...
      setDeploymentStatus(nextResult as unknown as Record<string, unknown>);
      setCurrentStep(5);
    } catch (err) {
...
const costPanelStyle: CSSProperties = {
  padding: '16px',
