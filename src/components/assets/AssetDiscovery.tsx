...
import type { AssetFiltersState } from '../../types/components';
...
function computeReputationScore(asset: AssetInfo) {
  let score = 0;
...
function getVolatilityBucket(change24h: number | undefined | null): AssetFiltersState['volatility'] | null {
...
function applyAdvancedFilters(
  ...
  marketDataMap: Record<string, AssetMarketData>,
) {
  let result = [...assets];
  ...
  if (filters.verification_level) {
    result = result.filter((a) => {
      const level = a.is_verified ? 'domain' : 'none';
...
export default function AssetDiscovery() {
  ...
  const [marketDataMap, setMarketDataMap] = useState<Record<string, AssetMarketData>>({});
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [filters, setFilters] = useState<AssetFiltersState>(DEFAULT_FILTERS);
  const [activeTab, setActiveTab] = useState<'popular' | 'all' | 'search' | 'recommendations'>('popular');
  ...
  const loadAssets = async (cursor: string | null = null, append = false) => {
    try {
      setLoading(true);
      ...
      const horizonFilters: AssetSearchFilters = {
        ...
        sort_by: filters.sort_by as AssetSearchFilters['sort_by'],
        order: filters.order as AssetSearchFilters['order'],
  ...
  const handleFilterChange = (newFilters: Partial<AssetFiltersState>) => {
  ...
  const handleTabChange = (tab: 'popular' | 'all' | 'search' | 'recommendations') => {
    setActiveTab(tab);
    addBreadcrumb('Asset discovery tab changed', 'navigation', { tab });
  };

  // Volatility badge helper for display in list
  const getVolatilityIndicator = (change24h: number | undefined | null) => {
