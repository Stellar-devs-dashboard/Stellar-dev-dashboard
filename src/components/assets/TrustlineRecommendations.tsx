...
import {
  ...
  type NetworkName,
  type TrustlineRecommendation,
} from '../../lib/stellar';
import { addBreadcrumb } from '../../lib/errorReporting';
import AssetCard from './AssetCard';
import { ResponsiveGrid } from '../layout/ResponsiveContainer';

export interface TrustlineRecommendationsProps {
  accountId: string;
  network: NetworkName;
}

export default function TrustlineRecommendations({ accountId, network }: TrustlineRecommendationsProps) {
  const [recommendations, setRecommendations] = useState<TrustlineRecommendation[]>([]);
...
interface RecommendationCardProps {
  recommendation: TrustlineRecommendation;
  network: NetworkName;
}

// Individual recommendation card component
function RecommendationCard({ recommendation, network }: RecommendationCardProps) {
  const { isMobile } = useResponsive();
  const { asset, recommendation_score, reasons, risk_factors } = recommendation;
